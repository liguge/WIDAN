...
class Harmonice_fast(nn.Module):
    ...
    def __init__(self, out_channels, kernel_size, frequency, eps=0.3, mode='sigmoid'):
        super(Harmonice_fast, self).__init__()
        self.out_channels = out_channels
        self.kernel_size = kernel_size
        self.eps = eps
        self.mode = mode
    ...
    def Harmonicev1(self, p):
        ...
        if self.mode == 'vanilla':
            return (torch.sin(0.03 / torch.sqrt(q) * 2 * w * (p - 0.1)) - torch.sin(
                0.03 / torch.sqrt(q) * w * (p - 0.1))) / (0.03 / torch.sqrt(q) * w * (p - 0.1))

        if self.mode == 'maxmin':
            a = (torch.sin(0.03 / torch.sqrt(q) * 2 * w * (p - 0.1)) - torch.sin(
                0.03 / torch.sqrt(q) * w * (p - 0.1))) / (0.03 / torch.sqrt(q) * w * (p - 0.1))
            return (a - a.min()) / (a.max() - a.min())
        ...
        if self.mode == 'tanh':
            return (1 / math.e) * torch.exp(((-0.03 / (torch.sqrt(q))) * (w * (p - 0.1))).tanh()) * (
                torch.sin(w * (p - 0.1)))
    ...
    # def Harmonicev2(self, p):
    #     return (torch.cos(4 * torch.pi * p) - torch.cos(2 * torch.pi * p)) / (2 * torch.pi * p)
    #
    # def Harmonicev3(self, p):
    #     k = 0.1
    #     return (torch.cos(4 * torch.pi * p) - torch.cos(2 * torch.pi * p)) / (2 * torch.pi * p) * 2 * torch.exp(
    #         (-1 * k ** k ** p ** p).sigmoid())
    #
    # def Harmonicev4(self, p):
    #     return (torch.cos(4 * torch.pi * p) - torch.cos(2 * torch.pi * p)) / (2 * torch.pi * p) * 2 * torch.exp(
    #         (-1 / (2 * p ** p)).sigmoid())

    def Harmonicev5(self, p):
        return (torch.sin(4 * torch.pi * p) - torch.sin(2 * torch.pi * p)) / (2 * torch.pi * p) * 2 * -(
                (1 / (pow(2 * torch.pi, 0.5))) * p * (torch.exp(((-torch.pow(p, 2)) / 2))).sigmoid())
    def forward(self):

        p1 = (self.time_disc - self.b_) / (self.a_ + self.eps)
        return self.Harmonicev5(p1).view(self.out_channels, 1, self.kernel_size)
# if __name__ == '__main__':
#     import numpy as np
#     import matplotlib.pyplot as plt
#
#     input = torch.randn(2, 1, 1024).cuda()
#     weight = Harmonice_fast(out_channels=64, kernel_size=300, eps=0.3, mode='vanilla', frequency=50000).forward().cuda()
...
#     plt.show()
# class Harmonice_fastv2(nn.Module):
#
#     def __init__(self, out_channels, kernel_size, frequency, eps=0.3, mode='sigmoid'):
#         super(Harmonice_fastv2, self).__init__()
#         self.out_channels = out_channels
#         self.kernel_size = kernel_size
#         self.eps = eps
#         self.mode = mode
...
#
#         if self.mode == 'vanilla':
#             return (torch.sin(0.03/torch.sqrt(q)*2*w*(p-0.1))-torch.sin(0.03/torch.sqrt(q)*w*(p-0.1))) / (0.03/torch.sqrt(q)*w*(p-0.1))
#
#         if self.mode == 'maxmin':
#             a = (1/math.e) * torch.exp(((-0.03 / (torch.sqrt(q))) * (w * (p - 0.1))).sigmoid()) * (
#                 torch.sin(w * (p - 0.1)))
...
#         Harmonice_right = self.Harmonicev4(p1)
#         Harmonice_left = self.Harmonicev4(p2)
#         return torch.cat([Harmonice_left, Harmonice_right], dim=1).view(self.out_channels, 1, self.kernel_size)
